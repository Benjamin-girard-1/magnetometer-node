...
def fit_state(rows: list[dict[str, float | str]], state: str) -> tuple[np.ndarray, np.ndarray, float, float]:
    ...
    means = np.asarray([float(row["ch2_mv_mean"]) for row in state_rows], dtype=float)
    zero_row = min(state_rows, key=lambda row: abs(float(row["current_a"])))
    zero_mean = float(zero_row["ch2_mv_mean"])
...
def main() -> None:
    ...
    csv_paths = sorted(
        (path for path in DATA_DIR.glob("*mA*.csv") if re.fullmatch(r"([+-]?\d+)mA_?(SR|R)\.csv", path.name)),
        key=lambda path: parse_filename(path),
    )
    ...
    if not rows:
        raise RuntimeError(f"No X-axis calibration CSV files found in {DATA_DIR}")
    ...
    out_csv = DATA_DIR / "x_axis_calibration_summary.csv"
    ...
    plt.title("Calibration de sensibilite de l'axe X - CH2 - carte 2")
    plt.xlabel("Champ applique par les bobines de Helmholtz selon X (uT)")
    plt.ylabel("Valeur mesuree - champ ambiant (mV)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()

    out_png = DATA_DIR / "x_axis_ch2_calibration.png"
    plt.savefig(out_png, dpi=160)

    print(f"Wrote {out_csv}")
    print(f"Wrote {out_png}")
    print(f"CH2 SR sensitivity: {sr_slope:.6f} mV/uT")
    print(f"CH2 R sensitivity: {r_slope:.6f} mV/uT")
