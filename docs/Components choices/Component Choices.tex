...
% Branch 1: +3.3VA
\node[rail, below left=1.5cm and 4.25cm of usb] (ldo33) {TPS7A2033\\(LDO)};
...
\node[rail, below=1.5cm of m55_I] (m55) {–6.2V};
...
% Branch 5: +5VA
\node[rail, left=0.5cm  of ldo9] (ldop5_i) {TPS7A4901\\(LDO)};
\node[rail, below=1.5cm of ldop5_i] (ldop5) {+5VA};
...
\begin{table}[htbp]
\centering
\caption{Estimated current budget by rail.}
\label{tab:power-budget}
\begin{tabular}{l l c c c c}
\toprule
\textbf{Rail} & \textbf{Part} & \shortstack{\textbf{Avg. current}\\\textbf{sensing (mA)}} 
& \shortstack{\textbf{Avg. current}\\\textbf{USB read (mA)}} & \shortstack{\textbf{Peak current}\\\textbf{(mA)}} \\
\midrule
+3.3\,V & ESP32-S3-WROOM-1              & 60   & 40  & 344 \\
        & MAX-M10                       & 10   & 10  & 100 \\
        & microSD (writes)              & 40   & 70  & 250 \\
        & AD7779                        & 30   &  0  &  50 \\
        & MCP4728                       &  2   &  0  &   2 \\
        & IMU                           &  5   &  5  &  25 \\
        & USB2641.                      &  1   & 65  &  70 \\
        & Misc. (LEDs, logic)           &  5   &  5   & 25 \\
\cmidrule(l){2-5}
        & Subtotal                      & \textbf{153}  & \textbf{195}  & \textbf{866} \\
\midrule
+5\,V   & INA851 (3 ch, +$V_\mathrm{s}$)& 21  & 0   &  27 \\
        & LM324DT                       & 1   & 0   &   3 \\
        & Offset strap drivers          & 19  & 0   & 156 \\
\cmidrule(l){2-5}
        & Subtotal                      & \textbf{41} & \textbf{0} & \textbf{186} \\
\midrule
-5\,V   & INA851 (3 ch, +$V_\mathrm{s}$)& 21  & 0   &  27 \\
        & LM324DT                       & 1   & 0   &   3 \\
        & Offset strap drivers          & 19  & 0   & 156 \\
...
+9\,V   & HMC1001                       & 15  & 0 & 15 \\
        & HMC1002                       & 30  & 0 & 30 \\
\cmidrule(l){2-5}
        & Subtotal                      & \textbf{45}  & \textbf{0} & \textbf{45} \\
\addlinespace
\midrule
\textbf{Total}  &  & \textbf{1 320 mW} & \textbf{644 mW} & - \\
\bottomrule
\end{tabular}
\vspace{2mm}
\end{table}
...
of 283.33\si{\ohm}, this results in 32.1~mA and 321~mW in total. The 
...
\subsubsection{-5\,V Analog}
To supply the INA851 and the other analog stages with a negative rail, a \SI{-5}{\volt} analog supply
(\(-5\)VA) is generated from the \SI{5}{\volt} USB input. First, the TPS63700 is configured as an inverting
DC/DC converter to produce an intermediate rail of approximately \SI{-5.5}{\volt} (\(-5.5\)V\(_\text{PRE}\)),
providing sufficient headroom for post-regulation. This intermediate voltage is then regulated by the low-noise
TPS7A33 LDO down to \SI{-5}{\volt}, improving both line rejection and output noise.

The TPS63700 output voltage is programmed using the resistor divider \textbf{R2/R3} referenced to the internal
\(\,V_\text{REF}\approx \SI{1.213}{\volt}\). With \textbf{R2} from \textit{VREF} to \textit{FB} and \textbf{R3} from \textit{FB} to
\textit{VOUT}, the programmed output is:
\[
V_{\text{OUT}} = -V_{\text{REF}}\cdot\frac{R3}{R2}.
\]
The following Table~\ref{tab:-5va-bom} summarizes the passive components used in this design.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=1\textwidth]{TPS63700_circuit.png}
  \caption{TPS63700 circuit (taken from the TPS63700 datasheet, 
  Fig.~2, p.~9).}
  \label{fig:TPS63700}
\end{figure}
...
\begin{table}[!htbp]
\centering
\caption{-5VA rail: TPS63700 inverting pre-regulator and TPS7A33 LDO components
(\SI{5}{\volt} in $\rightarrow$ \SI{-5.5}{\volt} $\rightarrow$ \SI{-5}{\volt} out).} See figure \ref{fig:TPS63700} 
for a circuit exanple
\label{tab:-5va-bom}
...
& Chosen for BOM reuse\\
& D1   & Schottky, \SI{20}{\volt}, \SI{1}{\ampere}
& e.g.\ SS14 or equivalent (fast, low \(V_F\)). \\
& R1   & \SI{10}{\ohm}
& Input filter with C1\\
& R2   & \SI{150}{\kilo\ohm}
& Feedback divider (VREF $\rightarrow$ FB)\\
& R3   & \SI{681}{\kilo\ohm}
& Feedback divider (FB $\rightarrow$ VOUT). Sets \(V_\text{OUT}\approx\SI{-5.5}{\volt}\) using
\(V_{\text{OUT}} = -V_{\text{REF}}\cdot\frac{R3}{R2}\) with \(V_\text{REF}\approx\SI{1.213}{\volt}\). \\
& R4   & \SI{100}{\kilo\ohm}
& Feed-forward network series resistor \\
& C1   & \SI{100}{\nano\farad}
&  \\
& C2   & \SI{220}{\nano\farad}
& \\
& C3   & \SI{10}{\pico\farad}
& \\
& C4   & \SI{10}{\micro\farad}
& \\
& C5   & 4x\SI{4.7}{\micro\farad}
& \\
& C6   & \SI{4.7}{\nano\farad}
& \\
& R\(_\text{EN}\)    & \SI{100}{\kilo\ohm}
& EN pull-down \\
\midrule
TPS7A33
& C\(_\text{IN}\)    & \SI{10}{\micro\farad} \(\ge\)\SI{10}{\volt}
& \\
& C\(_\text{IN,HF}\) & \SI{100}{\nano\farad} \(\ge\)\SI{10}{\volt}
& \\
& C\(_\text{OUT}\)   & \SI{47}{\micro\farad} \(\ge\)\SI{10}{\volt}
&  \\
& C\(_\text{NR}\)    & \SI{100}{\nano\farad}
& Noise-reduction/soft-start capacitor\\
& C\(_\text{FF}\)   & \SI{10}{\nano\farad}
& \\
& R1   & \SI{491}{\kilo\ohm}
& \\
...
% Using L1 = 10 uH generally reduces inductor ripple current and can help EMI,
% but ensure peak current remains below the inductor saturation current at maximum load.
